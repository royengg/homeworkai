...
import { runLLM, generateBlueprint, generateSection } from "../services/analyze.service";
...
export async function processAnalyzeJob(job: Job<Jobs>) {
  ...
  try {
    ...
    const isAssignment = pdfText.length > 5000 || pdfText.toLowerCase().includes("assignment") || pdfText.toLowerCase().includes("syllabus");

    if (isAssignment) {
      logger.info("Assignment Master Mode (Resilient)", { jobId: job.id });
      ...
      if (!blueprint) {
        ...
        if (lastResult?.output) {
          ...
          logger.info("Resuming from a previous analysis record", { prevAnalysisId: lastResult.id });
        }
      }
      ...
      if (!blueprint) {
        ...
        logger.info("Resuming execution with blueprint", { title: blueprint.title, completedSections: expandedSections.length });
      }
        ...
        data: {
          output: {
            ...
            assignment: {
              ...
              full_content: expandedSections.length > 0 ? fullContentSoFar : "Generating sections..."
            }
          } as any
        },
        include: { upload: { include: { parseResult: true } } }
      ...
      for (const [index, section] of blueprint.sections.entries()) {
        const alreadyDone = expandedSections.find((s: any) => s.section_id === section.id);
        if (alreadyDone) {
          logger.info(`Section ${index + 1} already exists, skipping...`, { sectionId: section.id });
          continue;
        }

        logger.info(`Expanding section ${index + 1}/${blueprint.sections.length}`, { sectionId: section.id });
        await job.updateProgress(Math.floor(((index + 1) / blueprint.sections.length) * 100));
    ...
    await prisma.analysisResult.update({
      ...
      data: {
        ...
        error: isQuotaError ? "API Quota Exceeded. Please try again later. Progress has been saved!" : errorMessage
      }
    });
    throw error;
  }

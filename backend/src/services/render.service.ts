...
function renderMarkdown(doc: PDFKit.PDFDocument, text: string) {
  ...
  lines.forEach((line) => {
    ...
    if (
      ...
      (trimmed.startsWith("[") && trimmed.endsWith("]Diagram]"))
    ) {
...
function renderDiagramPlaceholder(doc: PDFKit.PDFDocument, text: string) {
  const label =
    text.replace(/\[DIAGRAM:|\]|```mermaid|```|Diagram/g, "").trim() ||
    "Technical Diagram";
  ...
  doc
    .roundedRect(50, currentY, width, height, 6)
    .lineWidth(0.5)
    .strokeColor("#D1D5DB")
    .stroke();

  doc
    .circle(50 + width / 2, currentY + 35, 12)
    .lineWidth(1.5)
    .strokeColor("#3B82F6")
    .stroke();
  doc
    .moveTo(50 + width / 2 - 8, currentY + 35)
    .lineTo(50 + width / 2 + 8, currentY + 35)
    .stroke();
  doc
    .moveTo(50 + width / 2, currentY + 35 - 8)
    .lineTo(50 + width / 2, currentY + 35 + 8)
    .stroke();
  ...
  doc
    ...
    .font("Helvetica-Bold")
    .fillColor("#111827")
    .text(label.toUpperCase(), 50, currentY + 60, { align: "center", width });
  doc
    .fontSize(7)
    .font("Helvetica-Oblique")
    .fillColor("#6B7280")
    .text(
      "[ Logic Flowchart & Technical Architecture Visualization ]",
      50,
      currentY + 75,
      { align: "center", width },
    );
  doc
    ...
    .fillColor("#9CA3AF")
    .text(
      "Figure: Conceptual Diagram representing " + label,
      50,
      currentY + 90,
      { align: "center", width },
    );
...
export function renderOutputToPdfStream(data: any) {
  ...
  if (isAssignment && data.assignment) {
    ...
    doc
      ...
      .font("Helvetica-Bold")
      .text(
        sanitizeForPdf(assignment.title || "Untitled Assignment"),
        50,
        220,
    ...
    doc
      ...
      .font("Helvetica")
      .fillColor("#3B82F6")
      .text(sanitizeForPdf(subject), { align: "center" });
    if (topic) {
      doc.moveDown(0.5);
      doc
        .fontSize(12)
    ...
    doc
      ...
      .font("Helvetica")
      .fillColor("#4B5563")
      .text("Academic Comprehensive Assignment Solution", { align: "center" });
    ...
    doc
      .fontSize(10)
      .fillColor("#9CA3AF")
      .text(`Document Generated for Academic Purposes`, 0, 720, {
        align: "center",
        width: doc.page.width,
      });
    doc.text(`Reference Date: ${new Date().toLocaleDateString()}`, {
      align: "center",
    });

    const contentfulSections =
      assignment.sections && Array.isArray(assignment.sections)
        ? assignment.sections.filter(
            (s: any) => s && s.content && s.content.trim().length > 0,
          )
        : [];

    const hasBlueprintSections =
      assignment.blueprint?.sections &&
      assignment.blueprint.sections.length > 0;
    ...
    if (hasBlueprintSections || contentfulSections.length > 0) {
      if (hasBlueprintSections) {
        ...
        assignment.blueprint.sections.forEach((s: any, i: number) => {
          ...
          doc.text(` ${sanitizeForPdf(s.title || "Untitled Section")}`, {
            indent: 20,
          });
          doc.moveDown(0.5);
        });
      ...
      if (contentfulSections.length > 0) {
        ...
        contentfulSections.forEach((section: any, i: number) => {
          const blueprintSection = assignment.blueprint?.sections?.find(
            (s: any) => s.id === section.section_id,
          );
          ...
          doc
            ...
            .fillColor("#111827")
            .text(
              `${i + 1}. ${sanitizeForPdf(blueprintSection?.title || "Section")}`,
            )
            .moveDown(1);
            ...
            section.citations.forEach((c: any) => {
              if (c) {
                doc
                  ...
                  .font("Helvetica")
                  .fillColor("#6B7280")
                  .text(`• ${sanitizeForPdf(c)}`, { indent: 15 })
                  .moveDown(0.2);
              }
            });
          }
    ...
    doc
      .fontSize(10)
      .fillColor("#555")
      .text(`Document ID: ${data.document_id}`, { align: "center" });
    doc.moveDown(1.5);
    ...
    for (const q of data.questions || []) {
      doc
        ...
        .fontSize(14)
        .fillColor("#000")
        .text(`${q.qid} — ${sanitizeForPdf(q.question_text)}`);
      doc.moveDown(0.4);
      ...
      for (const p of q.parts || []) {
        doc
          .font("Helvetica-Bold")
          .fontSize(12)
          .text(sanitizeForPdf(`${p.label} Answer: ${p.answer}`));
        doc.moveDown(0.2);
  ...
  const docTitle = sanitizeForPdf(
    data.assignment?.title || data.document_id || "HomeworkAI Solution",
  );
  ...
  for (let i = range.start; i < range.start + range.count; i++) {
    ...
    if (i > range.start) {
      doc
        ...
        .fillColor("#9CA3AF")
        .font("Helvetica")
        .text(docTitle, 50, 30, { align: "left" });
    ...
    doc
      ...
      .font("Helvetica")
      .text(
        `HomeworkAI • Page ${i + 1} of ${range.count}`,
        0,
        doc.page.height - 35,
